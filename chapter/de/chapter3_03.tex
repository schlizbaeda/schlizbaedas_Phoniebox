...
Die Community ist sich uneins darüber, wie hochwertig die Audioqualität
einer {\Bezeichnung} sein sollte. Meine persönliche Meinung ist, dass
gerade Kinder noch ein wesentlich besseres Gehör haben als wir
Erwachsenen, sowohl hinsichtlich des Frequenzspektrums bei Hochtönen als
auch bei der Empfindlichkeit. Daher sollten mal 10 Euro Mehrausgaben für
die Kleinen nicht \textbf{das} Entscheidungskriterium sein. Die Freude
bei Groß und Klein wird sich bei einer qualitativ guten Umsetzung
länger halten. Allerdings gibt es auch Gründe, gerade für noch recht
...
Visaton-Laut\-spre\-chern (siehe Stückliste in Tabelle \ref{tab:bom})
...
Nach Änderungen an \filenam{/boot/config.txt} muss der {\RPi} neu
gebootet werden, damit die dort vorgenommenen Änderungen greifen:\\
\cmdPi{sudo reboot}\\
\cmdPC{ssh pi@phoniebox1}\comment{über \software{ssh} neu auf dem {RPi} anmelden}

Anschließend kann die Audioausgabe über den {\miniamp} überprüft
werden:\\
...
ein sogenanntes \textit{Rosa Rauschen} ertönen. Dieser Test kann durch
...
\cmdPi{alsamixer}\comment{Einstellung der Lautstärke}

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{software/alsamixer.png}
\caption{Lautstärkeeinstellung mit dem \software{alsamixer}}
\label{fig:alsamixer}
\end{figure}

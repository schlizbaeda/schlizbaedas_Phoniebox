...
und für batteriebetriebene \RPi-Projekte gut geeignet. Allerdings führte
bei mir ein zu langes Betätigen des On-/Off-Tasters dazu, dass der
{\onoffshim} die Spannungsversorgung nicht vollständig ausschaltete,
...
\comment{wurde so nicht durchgeführt!}

Diese Software war mir aber zu undurchsichtig, so dass ich lieber das
einfachere und (für mich) durchschaubare Verfahren des Anwenders 
...
einwandfrei funktioniert und ebenso verwendet werden könnte.

Grundsätzlich ist der Einschaltvorgang beim {\onoffshim} ein rein
hardwaremäßig umgesetztes Konzept, das keinerlei zusätzliche Software
benötigt. Beim Ausschalten ist eine Software erforderlich, die die
Betätigung des Tasters erkennt und das Betriebssystem vor dem Abschalten
...
\end{tabular}
\vspace{0.5cm}
\caption{Skripte für den \onoffshim}
\label{tab:onoffshim_scripts}
\end{table}


\subsection{Installation der Shellskripte für den \onoffshim}
Die beiden Shellskripte aus Tabelle \ref{tab:onoffshim_scripts} können
...
Nicht zwingend erforderlich, aber zur Bewahrung der Übersicht lohnt es
...
\subsection{Einträge in den aufrufenden Dateien vornehmen}
...
%fi\\
\\
\textcolor{red}{\# added by schlizbäda:\\
...
Das Skript \filenam{onoffshim\_switch.sh} wird beim Hochfahren über den
Eintrag in \filenam{/etc/rc.local} im Hintergrund gestartet und läuft
solange in einer Schleife, bis der Taster des {\onoffshim} an GPIO17
(Pin 11 der Stiftleiste) betätigt wird. Damit wird die Schleife
verlassen und das Herunterfahren des {\RPi} durch Aufruf des Kommandos
\texttt{poweroff} veranlasst.\\
\software{systemd} fährt den {\RPi} herunter. Im laufe dieses Prozesses
werden alle ausführbaren Dateien im Verzeichnis
\filenam{/lib/systemd/system-shutdown} gestartet, in dem sich unser
Shellskript \filenam{onoffshim\_gpio-shutoff.sh} befindet:\\
Zunächst wird über den GPIO27 (Pin 13, Variable \$cut\_pin) das Relais
angesteuert, um den Taster des {\onoffshim} künstlich zu trennen (siehe
Kapitel \ref{sect:onoffshim_relay}). Der GPIO17 (Pin 11, Variable
\$led\_pin) wird auf Ausgang umkonfiguriert, um zunächst in einer
for-Schlei\-fe die rote LED auf dem {\onoffshim} dreimal blinken zu
lassen. Schließlich wird der GPIO4 (Pin 7, Variable \$poweroff\_pin) als
